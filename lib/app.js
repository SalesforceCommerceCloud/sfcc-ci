...
function install(instance, appDef, sites, version, clientId) {
    ...
    return readOCAPIPermissions(appDef)
        ...
        .then(() => addOCAPIPermissions(instance, sites, OCAPIPermissions, clientId))
...
// clientId is optional & will be used to replace client_id in JSON data
function addOCAPIPermissions(instance, sites, permissions, clientId) {
    const promises = [];

    if (permissions.global) {
        permissions.global.forEach(permissionObj =>
            promises.push(patchOCAPIPermissions(instance, permissionObj, clientId)));
    }

    if (permissions.site) {
        sites.forEach(site => {
            permissions.site.forEach(permissionObj =>
                promises.push(patchOCAPIPermissions(instance, permissionObj, clientId, site)));
        });
    }
...
function patchOCAPIPermissions(instance, permissionObj, clientId, site) {
    const ocapiVersion = ocapi.getOcapiVersion(),
        token = auth.getToken();

    return new Promise((resolve, reject) => {
        const endpoint = site ? `/s/-/dw/data/${ocapiVersion}/sites/${site}/ocapiconfig/data` :
                                '/s/-/dw/data/${ocapiVersion}/ocapiconfig/data',
            options = ocapi.getOptions(instance, endpoint, token);

            if (clientId) {
                permissionObj.clients[0].client_id = clientId;
            }
