...
const TMP_DIR = os.tmpdir(),
    ...
    // file to store an installer version for reporting purposes
    INSTALL_METADATA_FILE = '.sfcc-ci-meta',

    // location of metrics collection service
    METRICS_SERVICE_URL = 'https://peaceful-springs-62474.herokuapp.com/api/installation';
...
class Installer {
    constructor(instance, appDef, sites, version, clientId, verbose) {
        ...
        // set of business object XML types encountered
        this.busObjsInstalled = new Set();
    }

    install() {
        let OCAPIPermissions;

        // clear any previous values from a prior installation
        this.busObjsInstalled.clear();
    ...
    getBusinessObjectFilePath(busObj, site) {
        ...
        // add to running list of business object types seen (for reporting)
        this.busObjsInstalled.add(busObjType);
        ...
        if (site) {
            const siteMappings = {
                ...
                'slot-configurations': path.join('sites', site, 'slots.xml'),
            };
        ...
        throw new Error(`Unknown ${site ? 'site' : ''} business object type: ${busObjType}`);
    }
    ...
    reportUsageMetrics() {
        const usageData = {
            client_id: auth.getClient(),
            installer_id: this.getInstallerId(),
            app_name: this.appDef.name || '',
            app_version: this.appDef.version || '',
            instance: this.instance,
            code_version: this.version,
            sites: this.sites.join(','),
            count_cartidges_installed: this.appDef.cartridges.length,
            list_cartidges_installed: this.appDef.cartridges.map(c => c.name).join(','),
            count_business_objects_installed: this.busObjsInstalled.size,
            list_business_objects_installed: [...this.busObjsInstalled].join(','),
            app_data: this.appDef,
        };

        const options = {
            url: METRICS_SERVICE_URL,
            headers: {
        ...
        request.post(options, (err, res) => {
            if (err) {
                console.warn('Error recording usage data');
            } else {
                if (res.statusCode >= 400) {
                    console.warn(`Error recording usage data (${res.statusCode})`);
                }
            }
        });
    }
    ...
    getInstallerId() {
        const metadataFile = path.join(__dirname, '..', '.sfcc-ci-meta');
        if (fse.existsSync(metadataFile)) {
            return fse.readFileSync(metadataFile, 'utf-8');
        } else {
            const randomId = Math.floor((1 + Math.random()) * 0x10000000000).toString(16);
            fse.writeFileSync(metadataFile, randomId);
            return randomId;
        }
    }
}
