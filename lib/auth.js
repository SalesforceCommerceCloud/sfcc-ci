...
const BUSINESS_MANAGER_TOKEN_PATH = '/dw/oauth2/access_token?client_id=%s';
...
/**
 * Obtain an access token with specific grant type for Business Manager user.
 * This makes the request directly to the instance (which will then forward it to AM).
 *
 * @param {String} instance (domain name) to send request to
 * @param {String} bmUser business manager username
 * @param {String} bmPassword business manager password
 * @param {String} client the client id to obtain the token for
 * @param {String} clientSecret the client secret associated with the client id
 * @return {Promise} the token
 */
function obtainBusinessManagerUserToken(instance, bmUser, bmPassword, client, clientSecret) {
    ...
    console.debug('Doing auth business manager request, payload: %s', JSON.stringify(payload));

    // build the request options
    const path = util.format(BUSINESS_MANAGER_TOKEN_PATH, client),
        options = getOptions(instance, path, `${bmUser}:${bmPassword}`, clientSecret, 'post', false);

    // the grant type as form data
    options['form'] = payload;

    // allow self-signed certificates, if needed (only supported for configuration via dw.json)
    // normally not needed in this module, but this request goes to the ecom server instead of the auth server
...
// TODO move this down into api section and make an equivalent function like auth is there now
// TODO unit tests
module.exports.obtainBusinessManagerUserToken = obtainBusinessManagerUserToken;
