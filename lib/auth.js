 ...
 */
function auth(client, clientSecret, user, userPassword, autoRenew, accountManager, callback) {
 ...
 * dw.json file located in the current working directory.
 * If the client secret is provided via parameter or dw.json, the Oauth authorization code grant is used.
 *
...
function login(client, clientSecret, authServer) {
    if (!client && dwjson['client-id']) {
        client = dwjson['client-id'];
        console.info('Using client id %s from dw.json at %s', dwjson['client-id'], process.cwd());
    }
...
module.exports.api = {
    ...
    clientAuth : (client_id, client_secret, user_name, user_password, renewal, callback) => {
        auth(client_id, client_secret, user_name, user_password, renewal, null, (err, res) => {
            if (typeof callback === 'function') {
                callback(err, res)
                return;
            }
