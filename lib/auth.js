...
const ACCOUNT_MANAGER_SSO_URL = AM_BASE + '/dwsso/oauth2/authorize?client_id=%s&redirect_uri=%s&response_type=code' +
    '&scope=openid+profile+roles+tenantFilter';
const ACCOUNT_MANAGER_SSO_URL_IMPLICIT = AM_BASE + '/dwsso/oauth2/authorize?client_id=%s&redirect_uri=%s' +
    '&response_type=token&scope=openid+profile+roles+tenantFilter';
        ...
        if ( parsed.query['code'] ) {
            ...
            obtainToken(basicEncoded, grantPayload, function (err, res) {
                ...
                if (!err && res.statusCode == 200) {
                    ...
                    response.end('Successfully authenticated. You can close this page.');
                } else {
                    console.error('Authentication failed: %s', err);
                    response.end('Authentication failed. Close this page and re-run the authentication.');
                    process.exitCode = 1;
                }
