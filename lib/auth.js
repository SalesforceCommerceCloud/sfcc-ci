...
function auth(client, clientSecret, user, userPassword, autoRenew, accountManager) {
    ...
    var flow = { grant : 'client_credentials', response_type : 'token' };

    if ( user || userPassword ) {
        console.warn('The Oauth "password" grant is about to be deprecated, please use the "client_credentials" grant instead. To execute a "client_credentials" grant, please use the command: $ sfcc-ci client:auth [client] [secret].');
        console.warn('We\'re now already using "client_credentials" grant for authentication!');
    }
