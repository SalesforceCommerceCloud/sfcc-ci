...
const CCDX_REQUESTED_SCOPES = ['openid', 'roles', 'tenantFilter', 'profile'];
...
function auth(client, clientSecret, user, userPassword, autoRenew, accountManager) {
    ...
    if ( flow['grant'] === 'password' ) {
        ...
        // add explicit scopes required for CCDX
        grantPayload['scope'] = CCDX_REQUESTED_SCOPES.join(' ');
    }
...
function login(client, clientSecret, authServer) {
    ...
    // append explicit scopes required for CCDX
    var optionalScopes = '&scope=' + CCDX_REQUESTED_SCOPES.join('+');
