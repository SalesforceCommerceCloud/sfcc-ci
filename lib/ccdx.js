...
const CCDX_SANDBOX_OPERATION_TIMEOUT = 1000 * 60 * 5; // 5 minutes
const CCDX_SANDBOX_HOST_PATTERN = '%s-%s.sandbox.exp.dx.unified.demandware.net';
const CCDX_SANDBOX_STATUS_POLL_TIMEOUT = 5000;
const CCDX_SANDBOX_STATUS_UP_AND_RUNNING = 'started';
...
function lookupSandbox(spec) {
    ...
    getAllSandboxes(function(err, list) {
        ...
        var filtered = list.filter(function(cand) {
            ...
            var candHost = util.format(CCDX_SANDBOX_HOST_PATTERN, cand.realm, cand.instance);
...
module.exports.cli = {
    ...
    create : function(realm, alias, asJson, sync, setAsDefault) {
        ...
        createSandbox(realm, function(err, newSandbox) {
            ...
            var newHost = util.format(CCDX_SANDBOX_HOST_PATTERN, newSandbox.realm,
                newSandbox.instance);
            var newAlias = ( alias ? alias : newHost);
            ...
            var timeout = setInterval(function() {
                ...
                if ( Date.now() - startTime > CCDX_SANDBOX_OPERATION_TIMEOUT ) {
                    fault = {
                ...
                // poll the status
                if ( !fault ) {
                    getSandbox(createdSandbox.id, function(err, sandbox) {
                        if (err) {
                            fault = {
                                message : err.message
                            };
                        ...
                        if (sandbox.state === CCDX_SANDBOX_STATUS_UP_AND_RUNNING ) {
                            finished = true;
                        }
                    });
                }
                ...
                if (finished || fault) {
                    ...
                    if (fault) {
                        ...
                        result['message'] = util.format('reation of new sandbox %s for realm %s finished. ' +
                            'Sandbox id is %s, status of sandbox is %s. You may use `sfcc-ci sandbox:list` to ' +
                            'check the status of the creation.', newSandbox.instance, newSandbox.realm, newSandbox.id,
                        newSandbox.state);
                    }
            ...
            }, CCDX_SANDBOX_STATUS_POLL_TIMEOUT);
        });
    },
    ...
    get : function(id, asJson, hostOnly, openBrowser) {
        getSandbox(id, function(err, sandbox) {
            ...
            if (hostOnly) {
                result['host'] = util.format(CCDX_SANDBOX_HOST_PATTERN, sandbox.realm, sandbox.instance);
                if (asJson) {
            ...
            if (openBrowser) {
                ...
                var bmUrl = 'https://' + util.format(CCDX_SANDBOX_HOST_PATTERN, sandbox.realm, sandbox.instance) +
                    BM_PATH;
