...
module.exports.cli = {
    ...
    list : function(page, size, asJson) {
        listClients(page, size, function(err, result) {
            if (err) {
                if (asJson) {
                    console.json({error: err.message});
                } else {
            ...
            // table fields
            const table = result.content.map(entry => ([ entry.id, entry.name, entry.active,
                entry.organizations.join(', ')]));
            table.unshift(['id','name','active','organizations']);
