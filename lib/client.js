 ...
 * @param {Function} callback the callback to execute, the error and the Oauth client are available as arguments to the callback function
 */
function listClients(page, size, callback) {
    // build the request options
    ...
    request(options, function (err, res, body) {
        ...
        // do the callback with the body47ccb7e7-dd4d-4434-8d1b-a95ae5623ccf
        callback(undefined, body);
    });
}
...
/**
 * adds an existing Oauth client.
 *
 * @param {Object} client the Oauth client to update
 * @param {Object} changes the changes to make to the Oauth client
 * @param {Function} callback the callback to execute, the error and the updated Oauth client are available as arguments to the callback function
 */
function addClient(clientDefiniton, callback) {
    // build the request options
    var options = getOptions(API_BASE + '/apiclients/', auth.getToken(), 'POST');

    // the payload
    options['body'] = clientDefiniton;
    // do the request
    request.post(options, function (err, res, body) {
        ...
        if ( errback ) {
            ...
            callback(new Error(`Adding Oauth client failed: ${err}`));
            return;
        } else if (res.statusCode >= 400 && body['errors']) {
            callback(new Error(`Adding Oauth client failed: ${body['errors'][0]['message']}`));
            return;
        } else if (res.statusCode >= 400) {
            callback(new Error(`Adding Oauth client failed: ${res.statusCode}`));
            return;
        }
...
module.exports.cli = {
     ...
     *
     * @param {Object} clientDefinition changes to the Oauth client details
     * @param {String} file filename of a file based configuration
     * @param {Boolean} asJson optional flag to force output in json, false by default
     */
    add : function(clientDefinitionParam, file, asJson) {
        var clientDefinition = {}; 
        if (!clientDefinitionParam && file) {
            clientDefinition = JSON.parse(fs.readFileSync(file, 'utf-8'));
        } else {
            clientDefinition = clientDefinitionParam;
        }

        addClient(clientDefinition, function(err, updatedClient) {
            if (err) {
                if (asJson) {
                    console.json({error: err.message});
                } else {
            ...
            // the result
            var result = {
                message : `Oauth client has been added.`,
                id: updatedClient.id
            };
            ...
            console.info(result['message'], `Generated client ID ${result['id']}`);
        });
    ...
    list : function(page, size, asJson) {
        listClients(page, size, function(err, client) {
            if (err) {
                if (asJson) {
                    console.json({error: err.message});
                } else {
            ...
            const table = client.content.map(entry => ({id: entry.id, active: entry.active, name: entry.name, orgs: entry.organizations.join(',')}))
            console.table(table);
            console.table(client.links);
