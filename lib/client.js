...
/**
 * Retrieves audit logs for an Oauth client
 *
 * @param {String} id the client id
 * @param {Promise}
 */
async function getAuditLogs(id) {
    // build the request options
    var options = getOptions(API_BASE + '/apiclients/' + id + '/audit-log-records',
        auth.getToken(), 'GET');
...
module.exports.cli = {
     ...
     *
     * @param {String} id the id of the Oauth client
     * @param {Boolean} auditlogs optional flag to return audit logs for an Oauth client, false by default
     * @param {Boolean} asJson optional flag to force output in json, false by default
     */
    info : function(id, auditlogs, asJson) {
        // audit logs are requested
        if (auditlogs) {
            getAuditLogs(id).then(list => {
                if (asJson) {
                    console.json(list);
                    return;
                }
