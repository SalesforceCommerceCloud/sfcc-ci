...
function createClient(client, callback) {
    ...
    request.post(options, function (err, res, body) {
        ...
        if ( errback ) {
            ...
            callback(new Error(`Creating Oauth client failed: ${err}`));
            return;
        } else if (res.statusCode >= 400 && body['errors']) {
            callback(new Error(`Creating Oauth client failed: ${body['errors'][0]['message']}`));
            return;
        } else if (res.statusCode >= 400) {
            callback(new Error(`Creating Oauth client failed: ${res.statusCode}`));
            return;
        }
...
module.exports.cli = {
     ...
     * @param {Boolean} asJson optional flag to force output in json, false by default
     */
    create : function(client, file, asJson) {
        ...
        createClient(clientDefinition, function(err, newClient) {
            if (err) {
                if (asJson) {
                    console.json({error: err.message});
                } else {
            ...
            // the result
            var result = {
                message : `Oauth client has been created.`,
                id: newClient.id
            };

            if (asJson) {
                console.json(newClient);
                return;
            }
