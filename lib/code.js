    ...
    activateVersion(instance, code_version, auth.getToken(), function (err, res) {
        ocapi.ensureValidToken(err, res, function(err, res) {
            if (!err && res.statusCode == 200 && !res.fault) {
                console.info('Custom code version %s activated on %s',
                    code_version, instance);
            } else if (res && res.body.fault.type == 'CodeVersionModificationException') {
                console.warn('Custom code version %s already active on %s',
                    code_version, instance);
            } else {
                console.error('Activating custom code version %s on %s has failed: %s (%s)',
    ...
    listVersions(instance, auth.getToken(), function (err, res) {
        ocapi.ensureValidToken(err, res, function(err, res) {
            if (!err && res.statusCode == 200) {
                ...
                console.error('Reading custom code versions on %s failed: %s (%s)',
