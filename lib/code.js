...
function activate(instance, code_version) {
    activateVersion(instance, code_version, auth.getToken(), function (err, res) {
        ocapi.ensureValidToken(err, res, function(err, res) {
            ...
            } else if (res && res.body.fault.type == 'CodeVersionModificationException') {
...
/**
 * Returns a Promise with code version of the instance based on the optional desired version.
 *
 * @param {String} instance the instance to retrieve code version from
 * @param {String} the version being determined, if not provided this will return
 *                 the currently active version.
 */
function getVersion(instance, targetVersion) {
    return new Promise((resolve, reject) => {
        listVersions(instance, auth.getToken(), function(err, res) {
            ocapi.ensureValidToken(err, res, function(err, res) {
                if (res) {
                    if (res.body && res.body.fault) {
                        reject(res.body.fault);
                    } else {
                        let activeVersion;
                        res.body.data.forEach(version => {
                            if (version.id === targetVersion || (!targetVersion && version.active)) {
                                resolve(version.id);
                                return;
                            }
                        });
                        reject('version ' + targetVersion + ' not found on instance!');
                    }
                } else {
                    reject(err);
                }
            }, function() {
                getVersion(instance, targetVersion)
