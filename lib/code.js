        ...
        deployCode(instance, archive, auth.getToken(), options, function(err, newVersion) {
            ...
            if (!err && activate) {
                ...
                if (instance.indexOf('cert.', 0) === 0) {
                    var patchedInstance = instance.substring(5);
                    console.debug(`Cert host name ${instance} detected. Using ${patchedInstance} for code activation.`);
                }
                module.exports.cli.activate(patchedInstance, newVersion);
            }
        });
    },
