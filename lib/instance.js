...
const endpointMeta = '/s/-/dw/meta/v1/rest';

function configure(instance, alias) {
    // progress
    ...
    request
        .get('https://' + instance + endpointMeta)
        .end(function (err, res) {
            ...
            if (res && res.statusCode != 404) {
                ...
                console.log('Instance "%s" configured successfully using alias "%s"', instance, alias);
            } else {
                console.error('Configuring instance "%s" failed: %s', instance, err);
            }
        });
}
...
function getAllInstances() {
    ...
    for(var value of config) {
        if(value[0].indexOf('SFCC_INSTANCES_') !== 0) continue;
...
function saveState(instance) {
    job.run(instance, 'sfcc-save-instance-state', {});
}

function resetState(instance) {
    job.run(instance, 'sfcc-reset-instance-state', {});
}
