    ...
    runJob(instance, job_id, request_doc, auth.getToken(), function (err, res) {
        ocapi.ensureValidToken(err, res, function(err, res) {
            if (!err && ( res.statusCode == 200 || res.statusCode == 202 ) && !res.fault) {
                ...
                console.info('Job execution id: %s', job_execution_id);
                console.info('Check the status of this job execution by running:');
                console.info();
                console.info('    sfcc-ci job:status "%s" "%s" -i %s', job_id, job_execution_id, instance);
                console.info();
            } else {
                console.error('Starting job "%s" on %s failed: %s (%s)',
...
function runSync(instance, job_id, request_doc) {
    runJob(instance, job_id, request_doc, auth.getToken(), function (err, res) {
        ocapi.ensureValidToken(err, res, function(err, res) {
            if (!err && ( res.statusCode == 200 || res.statusCode == 202 ) && !res.fault) {
                ...
                var timeout = setInterval(function() {
                    getStatus(instance, job_id, job_execution.id, auth.getToken(), function (err, res) {
                        ocapi.ensureValidToken(err, res, function(err, res) {
                            if (!err && res.statusCode == 200 && !res.fault) {
                                ...
                                console.error('Getting job execution failed: %s (%s)',
                    ...
                    if (!running || fault) {
                        ...
                        if (fault) {
                            console.error('Getting job execution "%s" for job "%s" on %s failed: %s (%s)',
                                job_execution.id, job_id, instance, fault.type, fault.message);
                            process.exitCode = 1;
                        } else {
                            console.info('Job "%s" finished. Status of job execution: %s (%s)',
            ...
            runSync(instance, job_id, request_doc);
        });
    });
}
...
function status(instance, job_id, job_execution_id, verbose, logfile, asJson) {
    getStatus(instance, job_id, job_execution_id, auth.getToken(), function (err, res) {
        ocapi.ensureValidToken(err, res, function(err, res) {
            if (!err && res.statusCode == 200 && !res.fault) {
                ...
                if (verbose) {
                    ...
                    for (var prop in job_execution) {
                        console.info('    %s : %s', prop, job_execution[prop]);
                    }
                    console.info('');
            ...
            status(instance, job_id, job_execution_id, verbose, logfile, asJson);
        });
    });
}
