    ...
    runJob(instance, job_id, request_doc, auth.getToken(), function (err, res) {
        ...
        ocapi.ensureValidToken(err, res, function(err, res) {
            if (!err && ( res.statusCode == 200 || res.statusCode == 202 ) && !res.fault) {
                ...
                console.error('Starting job "%s" on "%s" has failed: %s (%s)',
    ...
    runJob(instance, job_id, request_doc, auth.getToken(), function (err, res) {
        ocapi.ensureValidToken(err, res, function(err, res) {
            if (!err && ( res.statusCode == 200 || res.statusCode == 202 ) && !res.fault) {
                ...
                console.log('Job "%s" successfully started on "%s". Execution id is: %s',
                ...
                var timeout = setInterval(function() {
                    ...
                    if (!running || fault) {
                        ...
                        if (fault) {
                            ...
                            console.log('Job "%s" finished. Status of job execution: %s (%s)',
