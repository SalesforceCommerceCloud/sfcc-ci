...
const logFileDir = '/on/demandware.servlet/webdav/Sites/Impex/log/';
        ...
        .end(function (err, res) {
            ...
            ocapi.ensureValidToken(err, res, function(err, res) {
                if (!err && res.statusCode == 200 && !res.fault) {
                    ...
                    console.log('Status of job execution %s (%s): %s (%s)', job_execution_id, job_id, job_execution.execution_status, job_execution.status);
                    // verbose log output
                    if(verbose) {
                        console.log('');
                        ...
                        if(job_execution['is_log_file_existing']) {
                            console.log('    https://' + instance + logFileDir + job_execution['log_file_name']);
                        } else {
                    ...
                    // open log file in browser
                        ...
                        var logFileUrl = 'https://' + instance + logFileDir + job_execution['log_file_name'];
                        console.log('');
                        console.log('Opening browser to log file...');
                        require('open')(logFileUrl);
                    }
                    console.log('');
                } else {
