...
function logPollingStates(poller, job_id, job_execution, instance, failFast, asJson) {
    var result = {};
    if (poller.errorThresholdContinue) {
        console.debug('Polling job status failed. Polling error threshold not reached.');
    }
    if (poller.isTimeoutExceeded) {
        result = {
            message : 'Polling job timeout has been exceeded.'
        };
        console.error(result.message);
    }
    if (poller.errorThresholdExceeded) {
        result = {
            message : util.format('Polling job status "%s" on %s failed. Error ' +
            'threshold reached. Stop polling, the job may still run.', job_id, instance)
        };
        console.error(result.message);
    }
    if (poller.hasGeneralError) {
        result = {
            fault: res.body.fault,
            error: util.format('Running job "%s" on %s failed', job_id, instance)
        };
        console.error(result.error);
        console.debug(result.fault);
        if (asJson) {
    ...
    if (poller.hasEnded && !poller.hasGeneralError) {
        if (asJson) {
            console.json(job_execution);
            return;
        }
        ...
        ocapi.ensureValidToken(err, res, function(err, res) {
            if (!err && ( res.statusCode == 200 || res.statusCode == 202 ) && !res.fault) {
                ...
                let poller = new Poller(
                    ...
                    JOB_EXECUTION_STATUS_POLL_ERROR_THRESHOLD);

                poller.start(() => {
                    getStatus(instance, job_id, job_execution.id, auth.getToken(), function (err, res) {
                        ocapi.ensureValidToken(err, res, function(err, res) {
                            poller.errorResponse = err;
                            poller.hasEnded = (job_execution.status !== JOB_EXECUTION_STATUS_CODE_RUNNING);
                            logPollingStates(poller, job_id, job_execution, instance, failFast, asJson);
                        });
                        poller.next();
                    });
                });
