...
function ensureValidToken(err, res, success, repeat) {
    // token invalid
    if (res && res.body && res.body.fault && res.body.fault.type == 'InvalidAccessTokenException') {
        ...
        if (!auth.getAutoRenewToken()) {
            console.error('Authorization token missing or invalid. Please (re-)authenticate first by' +
                ' running ´sfcc-ci client:auth:renew´ and make sure, your client has access to the instance.');
            process.exitCode = 1;
        } else {
            // attempt to renew
        ...
        // invalid auth header
        console.error('Authorization token missing or invalid. Please authenticate first by running ' +
