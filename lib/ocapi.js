...
/**
 * Constructs the http request options and ensure shared request headers across requests, such as authentication.
 *
 * @param {String} host
 * @param {String} path
 * @param {String} token
...
function enhanceRequestOptions(requestOptions) {
 ...
 *
 * @param {Object} requestOptions - object with request parameter, like url or method
 */
function retryableCallNew(requestOptions) {
    const axios = require('axios').default;

    requestOptions = enhanceRequestOptions(requestOptions);
    ...
    let result = {
        data: {
            ...
            status: 'Success',
            data: {
                id: '7ec92875-3fe4-4e69-947d-be7a0796b5f6',
                realm: 'bbsv',
                instance: '159',
                versions: {},
                state: 'new',
                createdAt: '2021-01-01T19:01:37Z',
                createdBy: 'tschaerff@demandware.com',
                links: [
                    {
                        url: '',
                        name: ''
    ...
    //         console.log('retryableCallNew - then - response: ', response);
    //         // return response;
    //     })
    ...
    //                 // renew and callback and repeat over
    //                 auth.renew();
    //                 retryableCallNew(requestOptions);
    //             }
    //         } else if (code === 'EPROTO') {
    //             console.error('Network or certificate error');
    //         } else if (error.message === 'wrong tag') {
    //             console.error('Certificate error');
    //         } else if (code === 'ENOTFOUND') {
    //             console.error('Cannot resolve host name. Ensure you use a proper instance host name or an alias from ' +
    //                 'the instance configuration. Detailed error: %s', error.message);
    //         }
    //
    //         console.error('An error occured. Try running the command again with -D,--debug flag.');
    //         console.debug('Error code: %s, message: %s, stack: %s', code, error.message, error.stack);
    //
    //         throw error;
    //     });
}
...
module.exports.retryableCallNew = retryableCallNew;
