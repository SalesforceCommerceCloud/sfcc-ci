...
function install(instance, packageDef, sites, version) {
    ...
    return readOCAPIPermissions(packageDef)
        .then(permissionObjs => OCAPIPermissions = permissionObjs)
        .then(() => uploadCode(packageDef, instance, version))
...
function readOCAPIPermissions(packageDef) {
    ...
    if (packageDef.ocapi) {
        if (packageDef.ocapi.site) {
            promise = promise.then(() => {
                return Promise.all(packageDef.ocapi.site.map(permissionFile => {
                    return readOCAPIFile(path.join(packageDef.baseDir, permissionFile));
                }))
                .then(sitePermissionObjs => permissions.site.push(...sitePermissionObjs));
            });
        };

        if (packageDef.ocapi.global) {
            promise = promise.then(() => {
                return Promise.all(packageDef.ocapi.global.map(permissionFile => {
                    return readOCAPIFile(path.join(packageDef.baseDir, permissionFile));
                }))
