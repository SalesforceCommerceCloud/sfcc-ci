 ...
 */
function searchRoles(token, count, callback) {
    // the page size
    ...
    var options = getOptions(API_BASE + endpoint, token, 'GET');
 ...
 */
function getRole(instance, role, verbose, token, callback) {
    // build the request options
    var endpoint = '/s/-/dw/data/' + ocapi.getOcapiVersion() + '/roles/' + role +
        '?select=(**)&expand=users,permissions';
    var options = ocapi.getOptions(instance, endpoint, token, 'GET');
 ...
 */
function searchLocalRoles(instance, query, sortBy, count, token, callback) {
    // the page size
    var size = ROLE_LIST_PAGE_SIZE
    if (count) {
        size = Number.parseInt(count);
    ...
    var options = ocapi.getOptions(instance, endpoint, token, 'POST');
...
module.exports.api = {
     ...
     */
    list: function (token, count) {
        return new Promise((resolve, reject) => {
            // get all roles
            searchRoles(token, count, function (err, roleResult, list) {
                if (err) {
                    reject(err.message);
                    return;
                }
     ...
     */
    listLocal: function (instance, role, query, token, sortBy, count) {
        return new Promise((resolve, reject) => {
            // get details of a single user if login was passed
            if (typeof (role) !== 'undefined' && role !== null) {
                getRole(instance, role, false, token, function (err, role) {
                    if (err) {
                        reject(err.message);
                        return;
                    }
            ...
            // get all roles
            searchLocalRoles(instance, query, sortBy, count, token, function (err, roleResult, list) {
                if (err) {
                    reject(err.message);
                    return;
                }
...
module.exports.cli = {
    ...
    list: function (count, asJson) {
        // get all roles
        searchRoles(auth.getToken(), count, function (err, roleResult, list) {
            if (err) {
                if (asJson) {
    ...
    listLocal: function (instance, role, query, role, sortBy, count, asJson, verbose) {
        // get details of a single user if login was passed
        if (typeof (role) !== 'undefined' && role !== null) {
            getRole(instance, role, verbose, token, function (err, role) {
                if (err) {
                    if (asJson) {
                        console.json({ error: err.message });
                    } else {
        ...
        // get all roles
        searchLocalRoles(instance, query, sortBy, count, auth.getToken(), function (err, roleResult, list) {
            if (err) {
                if (asJson) {
                    console.json({ error: err.message });
                } else {
