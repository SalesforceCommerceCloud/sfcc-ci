...
function lookupSandbox(spec) {
    ...
    getAllSandboxes(function(err, list) {
        ...
        var filtered = list.filter(function(cand) {
            ...
            var candHost = util.format(AWS_SANDBOX_HOST_PATTERN, cand.realm, cand.instance);
...
module.exports.cli = {
    ...
    create : function(realm, alias, asJson, sync, setAsDefault) {
        ...
        createSandbox(realm, function(err, newSandbox) {
            ...
            var newHost = util.format(AWS_SANDBOX_HOST_PATTERN, newSandbox.realm,
                newSandbox.instance);
    ...
    get : function(id, asJson, hostOnly, openBrowser) {
        getSandbox(id, function(err, sandbox) {
            ...
            if (hostOnly) {
                result['host'] = util.format(AWS_SANDBOX_HOST_PATTERN, sandbox.realm, sandbox.instance);
                if (asJson) {
                    console.log(JSON.stringify(result));
                } else {
            ...
            if (openBrowser) {
                ...
                var bmUrl = 'https://' + util.format(AWS_SANDBOX_HOST_PATTERN, sandbox.realm, sandbox.instance) +
                    BM_PATH;
