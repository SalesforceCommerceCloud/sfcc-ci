 ...
 * @param {String} topic the topic to return details for
...
function getRealm(realmID, topic, callback) {
    ...
    var options = getOptions(API_BASE + '/realms/' + realmID + extension, auth.getToken(), 'GET');
...
function updateRealm(realmID, maxSandboxTTL, defaultSandboxTTL, callback) {
    // build the request options
    var options = getOptions(API_BASE + '/realms/' + realmID, auth.getToken(), 'PATCH');

    // the payload
    options['body'] = {};

    if (maxSandboxTTL) {
        options['body']['maxSandboxTTL'] = maxSandboxTTL.toFixed();
    }
    if (defaultSandboxTTL) {
        options['body']['defaultSandboxTTL'] = defaultSandboxTTL.toFixed();
    }
...
module.exports.cli = {
    realm : {
         ...
         * @param {String} showQuota show detailed quota information
         * @param {String} showUsage show detailed usage information
    ...
    get : function(spec, asJson, hostOnly, openBrowser, showOperations, showUsage, showSettings) {
        ...
        lookupSandbox(spec, function(err, foundSandbox) {
            ...
            // get all details incl. links
            // TODO should be available to sandbox found in list (API bug?)
