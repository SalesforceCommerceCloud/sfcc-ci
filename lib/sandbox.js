...
/**
 * checks the passed string for validity. Checks if the JSON string is syntactically correct
 * and does a basic semantic check for the same API client as used by the CLI client. Throws
 * an error if a check failed.
 *
 * @param {String} settingsAsJSON the settings as JSON string to validate
 * @return {Object} the validated and parsed JSON
 */
function validateAPISettings(settingsAsJSON) {
    var settings;
    // syntactical check
    try {
    ...
    // sematical check
    if (settings['client_id'] && settings.client_id === auth.getClient()) {
        console.debug(`Cannot patch existing permissions`);
        throw new Error(`Cannot patch existing permissions`);
    } else if (Array.isArray(settings)) {
        // in case of a multi perm set
        settings.forEach(function(permSet) {
            if (permSet['client_id'] && permSet.client_id === auth.getClient()) {
                console.debug(`Cannot patch existing permissions`);
                throw new Error(`Cannot patch existing permissions`);
            }
        });
...
function createSandbox(realm, ttl, additionalOcapiSettings, additionalWebdavSettings, callback) {
    ...
    if (additionalOcapiSettings) {
        try {
            var addition = validateAPISettings(additionalOcapiSettings);
            ocapiSettings = ocapiSettings.concat(addition);
        } catch (err) {
    ...
    if (additionalWebdavSettings) {
        try {
            var addition = validateAPISettings(additionalWebdavSettings);
            webdavPermissions = webdavPermissions.concat(addition);
        } catch (err) {
