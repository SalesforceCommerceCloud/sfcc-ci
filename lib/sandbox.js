...
const API_BASE = 'admin.dev.dx.unified.demandware.net/api/v1';
const API_SANDBOXES = API_BASE + '/sandboxes';
const API_SANDBOXES_OPERATION_TIMEOUT = 1000 * 60 * 5; // 5 minutes
const AWS_SANDBOX_HOST_PATTERN = '%s-%s.sandbox.dev.dx.unified.demandware.net';
    ...
    request.post(options, function (err, res, body) {
        ...
        if ( !errback ) {
            if (res.statusCode >= 400) {
                errback = new Error(util.format('Creating sandbox for realm %s failed: %s (%s)', realm,
                    body.error.message, res.statusCode));
            } else if (err) {
...
module.exports.cli = {
    ...
    get : function(id, asJson, hostOnly, openBrowser) {
        getSandbox(id, function(err, sandbox) {
            ...
            result['instance'] = instance.getInstanceDetails(getSandboxHost(sandbox));

            if (hostOnly) {
                if (asJson) {
                    console.log(JSON.stringify({'host' : result['instance']['host']}));
                } else {
