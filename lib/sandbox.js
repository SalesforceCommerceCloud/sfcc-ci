 ...
 */
function createSandbox(realm, ttl, profile, additionalOcapiSettings, additionalWebdavSettings, callback) {
    ...
    if (profile !== null) {
        if (SANDBOX_RESOURCE_PROFILES.indexOf(profile) === -1) {
            callback(new Error(`Invalid resource profile '${profile}', use on of: ${SANDBOX_RESOURCE_PROFILES.join(', ')}`));
            return;
        }
...
module.exports.cli = {
     ...
     */
    create : function(realm, alias, ttl, profile, ocapiSettings, webdavPermissions, asJson, sync, setAsDefault) {
        // memorize the start time and duration
        var startTime = Date.now();
        var duration = 0;

        createSandbox(realm, ttl, profile, ocapiSettings, webdavPermissions, function(err, newSandbox) {
            if (err) {
                if (asJson) {
                    console.json({error: err.message});
                } else {
