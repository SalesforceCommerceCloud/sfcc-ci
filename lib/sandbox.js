...
const API_SANDBOXES_OPERATION_TIMEOUT = 1000 * 60 * 8; // 8 minutes
...
const AWS_SANDBOX_STATUS_FAILED = 'failed';
...
module.exports.cli = {
    ...
    list : function(asJson, sortBy) {
        getAllSandboxes(function(err, list) {
            ...
            // table fields
            var data = [['ID','Realm','Instance','Version','State','CreatedAt','CreatedBy']];
            for (var i of list) {
                ...
                if ( !fault ) {
                    getSandbox(newSandbox.id, function(err, sandbox) {
                        ...
                        if (sandbox && sandbox.state === AWS_SANDBOX_STATUS_UP_AND_RUNNING ) {
                            finished = true;
                        } else if (sandbox && sandbox.state === AWS_SANDBOX_STATUS_FAILED ) {
                            fault = {
