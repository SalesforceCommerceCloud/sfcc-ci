...
function logPollingStates(poller, newSandboxResponse, sandbox, errors, callback) {
...
function sandboxPolling(newSandboxResponse, callback) {
    ...
    poller.start(() => {
        getSandbox(newSandboxResponse.id, null, function(err, sandbox) {
            poller.isTimeoutExceeded = Date.now() - newSandboxResponse.timings.startTime > API_SANDBOXES_OPERATION_TIMEOUT;
            poller.errorResponse = err;
            poller.next();
            logPollingStates(poller, newSandboxResponse, sandbox, err, function(pollingResults) {
