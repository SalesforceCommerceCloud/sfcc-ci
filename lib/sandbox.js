 ...
 */
function getAllSandboxes(includeDeleted) {
    let requestOptions = {
        url: 'https://' + API_SANDBOXES + '?include_deleted=' + ( includeDeleted ? 'true' : 'false' ),
        method: 'GET',

    };

    return ocapi.retryableCallNew(requestOptions)
...
function lookupSandbox(spec, callback) {
    ...
    getSandbox(spec['id'], null)
        .then(function(response) {

            console.log('lookupSandbox - then - response: ', response);
 ...
 */
function createSandbox(realm, ttl, additionalOcapiSettings, additionalWebdavSettings) {
    ...
    if (additionalOcapiSettings) {
        try {
            ...
            return new Promise(function(resolve, reject) {
                reject(new Error(`Invalid OCAPI settings: ${err.message}`));
            });
        }
    }
    ...
    if (ttl !== null && !isNaN(ttl)) {
        ...
        options['data']['ttl'] = ttl.toFixed();
    }

    return ocapi.retryableCallNew(options)
        .then(function(response) {
        ...
        .catch(function(error) {
            let code = error.code ? error.code : error.response.status;
            let newError = null;

            if (code >= 400) {
                newError = new Error(util.format('Creating sandbox for realm %s failed: %s', realm,
                    formatError(error.toJSON().message, code)));
            } else {
                newError = new Error(util.format('Creating sandbox for realm %s failed: %s', realm, error));
            }
...
function getSandbox(id, topic) {
    ...
    // do the request
    let requestOptions = {
        url: 'https://' + API_SANDBOXES + '/' + id + extension,
        method: 'GET'
    };
    ...
    return ocapi.retryableCallNew(requestOptions)
        ...
        .catch(function(error) {
            ...
            if (code >= 400) {
                newError = new Error(util.format('Retrieving details for sandbox failed: %s',
                    formatError(errorJson.message, code)));
            } else {
                newError = new Error(util.format('Retrieving details for sandbox failed: %s', error));
            }
...
module.exports.cli = {
    ...
    list : function(showDeleted, asJson, sortBy) {
        return getAllSandboxes(showDeleted)
            .then(function(list) {
                if (sortBy) {
                    list = require('./json').sort(list, sortBy);
                }
                ...
                console.error(util.format('Retrieving list of sandboxes failed with error: %s', error.message));
            });
    },
    ...
    create : function(realm, alias, ttl, ocapiSettings, webdavPermissions, asJson, sync, setAsDefault) {
        ...
        return createSandbox(realm, ttl, ocapiSettings, webdavPermissions)
            .then(function(newSandbox) {
                // console.log('create - then - response: ', response);
                ...
                if ( setAsDefault ) {
                    instance.config.setDefault(newHost)
                        .then(function() {
                            result['instance']['default'] = true;
                            result['message'] += ' New sandbox set as default instance.';
                        })
                        .catch(function(error) {
                            console.error('Cannot set unknown "%s" as default instance.', newHost);
                ...
                var timeout = setInterval(function() {
                    ...
                    getSandbox(newSandbox.id, null)
                        ...
                        .catch(function(error) {
                            if (error && errorThreshold > 0) {
                            ...
                            } else if (error && errorThreshold === 0) {
                                ...
                                fault = {
                                    message : 'Polling sandbox status failed. Error threshold reached. Stop polling, ' +
                                        'the creation may still continue. Detailed error was ' + error.message
                                };
                            }
                        });
