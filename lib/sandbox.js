 ...
 * @param cert true, to automatically generate a certificate for the unique alias
 ...
 */
function registerForSandbox(sbxID, alias, unique, cert, callback) {
    // the payload
    var options = ocapi.getOptions('POST', API_SANDBOXES + '/' + sbxID + '/aliases');
    options['body'] = {name: alias, unique: !!(unique || cert) , requestLetsEncryptCertificate: !!cert};
    ocapi.retryableCall('POST', options, function (err, res) {
        if (err) {
...
module.exports.cli = {
    ...
    alias : {
         ...
         * @param {Boolean} asJson optional flag to force output in json, false by default
         * @param {Boolean} cert optional flag to automatically generate a certificate for a unique alias, false by default
         */
        create : function (spec, alias, unique, asJson, cert) {
            runForSandbox(spec, asJson, function (sandbox) {
                registerForSandbox(sandbox.id, alias, unique, cert, function (err, result) {
                    if (err) {
                        if (asJson) {
                            console.json({error: err.message});
                        } else {
