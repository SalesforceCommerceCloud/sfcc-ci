...
function createLocalUser(instance, login, user, callback) {
    // error, in case neither a password nor an external id has been provided
    // as this case is not handled on the server
    if ( !user['password'] && !user['external_id'] ) {
        callback(new Error(util.format('Creating user %s failed: Provide a user password or an external id', login)));
        return;
    }
