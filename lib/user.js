...
function toExternalUser(user) {
    ...
    if ( user['roleTenantFilter'] !== undefined && user['roleTenantFilter'] !== null ) {
 ...
 *
 * @param {Object} user to reset
 * @param {String} token oauth token
 * @param {Function} callback to execute, the error and the user are available as arguments to the callback function
 */
function resetUser(user, token, callback) {

    // build the request options
    var options = getOptions(API_BASE + '/users/' + user['id'] + '/reset', auth.getToken(), 'POST');
    ...
    request(options, function (err, res, body) {
        ...
        if (errback) {
            ...
            callback(new Error(util.format('Reseting user failed: %s', err)));
            return;
        } else if (res.statusCode >= 400 && body['errors']) {
            callback(new Error(util.format('Reseting user failed: %s', body['errors'][0]['message'])));
            return;
        } else if (res.statusCode >= 400) {
            callback(new Error(util.format('Reseting user failed: %s', res.statusCode)));
            return;
        }
...
module.exports.cli = {
    ...
    * Reset a user.
    *
    * @param {Object} login the user to reset
    * @param {Boolean} asJson optional flag to force output in json, false by default
    * @param {String} token oauth token
    * @param {Function} callback callback function to call when called through the Javascript API
    */
    reset: function (login, asJson, token, callback) {
        getUser(login, token, function (err, user) {
            if (err) {
                if (typeof callback !== 'undefined') {
                    callback(err, undefined);
            ...
            resetUser(user, token, function (err) {
                if (err) {
                    if (typeof callback !== 'undefined') {
                        callback(err, undefined);
                        return;
                ...
                // the result
                var result = {
                    message: util.format('User %s %s.', login, 'resetted'),
                };
...
module.exports.api = {
     ...
     * @param {String} token oauth token
     */
    reset: (login, token) => new Promise((resolve, reject) => {
        module.exports.cli.reset(login, false, token, (err, result) => {
            if (err) {
                reject(err);
                return;
            }
