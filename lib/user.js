...
const USER_ALLOWED_READ_PROPERTIES = [ 'id', 'userState', 'roles', 'roleTenantFilter', 'primaryOrganization', 'mail',
    'firstName', 'lastName', 'displayName', 'organizations' ];
 ...
 */
function getUsers(org, role, count, callback) {
    // the page size
 ...
 * @param {Function} callback the callback to execute, the error and the user are available as arguments to the callback function
 */
function deleteUser(user, purge, callback) {
    // for purging we fire a full DELETE request
    if (purge) {
        // build the request options
        var options = getOptions(API_BASE + '/users/' + user['id'], auth.getToken(), 'DELETE');
 ...
 * @param {Function} callback the callback to execute, the error and the created user are available as arguments to the callback function
 */
function createLocalUser(instance, login, user, callback) {
    ...
    // build the request options
    var options = ocapi.getOptions(instance, endpoint, auth.getToken(), 'PUT');
    ...
    request(options, function (err, res, body) {
        ...
        // do the callback with the body
        // filter some properties before returning
        callback(undefined, filterLocalUser(body));
    });
}
 ...
 */
function updateLocalUser(instance, user, changes, callback) {
    // build the request options
    var endpoint = '/s/-/dw/data/' + ocapi.getOcapiVersion() + '/users/' + user['login'];

    // build the request options
    var options = ocapi.getOptions(instance, endpoint, auth.getToken(), 'PATCH');
 ...
 */
function grantLocalRole(instance, login, role, callback) {
    // build the request options
 ...
 * @param {Function} callback the callback to execute, the error and the changed user are available as arguments to the callback function
 */
function revokeLocalRole(instance, login, role, callback) {
    // build the request options
    var endpoint = '/s/-/dw/data/' + ocapi.getOcapiVersion() + '/roles/' + role + '/users/' + login;
 ...
 */
function deleteLocalUser(instance, login, state, callback) {
    // build the request options
    var endpoint = '/s/-/dw/data/' + ocapi.getOcapiVersion() + '/users/' + login;
 ...
 */
function searchLocalUsers(instance, query, role, sortBy, count, start, callback) {
    // the page size
    var size = USER_LIST_PAGE_SIZE
    if ( count ) {
        size = Number.parseInt(count);
...
module.exports.filterLocalUser = filterLocalUser;
module.exports.cli = {
    /**
     * Creates a new user.
     *
    ...
    create : function(org, user, mail, firstName, lastName, asJson) {
        ...
        libOrg.getOrg(org, function(err, foundOrg) {
            if (err) {
                if (asJson) {
     ...
     */
    list : function(org, role, login, count, asJson, sortBy) {
        ...
        if ( org ) {
            libOrg.getOrg(org, function(err, foundOrg) {
                ...
                getUsers(foundOrg['id'], role, count, getUsersCallback);
            });
            return;
        }
        // no org was passed
        getUsers(null, role, count, getUsersCallback);
    },
     ...
     * @param {Boolean} asJson optional flag to force output in json, false by default
     */
    update : function(login, changes, asJson) {
        getUser(login, function(err, user) {
            if (err) {
                if (asJson) {
                    console.json({error: err.message});
            ...
            updateUser(user, changes, function(err, updatedUser) {
                if (err) {
                    if (asJson) {
                        console.json({error: err.message});
                    } else {
     ...
     * @param {Boolean} asJson optional flag to force output in json, false by default
     */
    delete : function(login, purge, asJson) {
        getUser(login, function(err, user) {
            if (err) {
                if (asJson) {
                    console.json({error: err.message});
            ...
            deleteUser(user, purge, function(err) {
                if (err) {
                    if (asJson) {
                        console.json({error: err.message});
                    } else {
     ...
     * @param {Boolean} asJson optional flag to force output in json, false by default
     */
    createLocal : function(instance, login, user, asJson) {
        if (typeof(user) === 'undefined' || user === null) {
        ...
        createLocalUser(instance, login, user, function(err, newUser) {
            if (err) {
                if (asJson) {
                    console.json({error: err.message});
                } else {
     ...
     * @param {Boolean} asJson optional flag to force output in json, false by default
     */
    searchLocal : function(instance, login, query, role, sortBy, count, start, asJson) {
        // get details of a single user if login was passed
        if ( typeof(login) !== 'undefined' && login !== null ) {
        ...
        // get all users
        searchLocalUsers(instance, query, role, sortBy, count, start, function(err, result, list) {
            if (err) {
                if (asJson) {
                    console.json({error: err.message});
                } else {
     ...
     * @param {Boolean} asJson optional flag to force output in json, false by default
     */
    updateLocal : function(instance, login, changes, asJson) {
        ...
        getLocalUser(instance, login, function(err, user) {
            ...
            updateLocalUser(instance, user, changes, function(err, updatedUser) {
                if (err) {
                    if (asJson) {
                        console.json({error: err.message});
                    } else {
     ...
     * @param {Boolean} asJson optional flag to force output in json, false by default
     */
    grantLocal : function(instance, login, role, asJson) {
        if (typeof(instance) === 'undefined' || instance === null) {
            console.error('Missing instance. Please pass an instance using -i,--instance');
            return;
        }
        ...
        getLocalUser(instance, login, function(err, user) {
            ...
            grantLocalRole(instance, login, role, function(err, changedUser) {
                if (err) {
                    if (asJson) {
                        console.json({error: err.message});
                    } else {
     ...
     * @param {Boolean} asJson optional flag to force output in json, false by default
     */
    revokeLocal : function(instance, login, role, asJson) {
        if (typeof(instance) === 'undefined' || instance === null) {
            console.error('Missing instance. Please pass an instance using -i,--instance');
            return;
        }
        ...
        getLocalUser(instance, login, function(err, user) {
            ...
            revokeLocalRole(instance, login, role, function(err, changedUser) {
                if (err) {
                    if (asJson) {
                        console.json({error: err.message});
                    } else {
     ...
     * @param {Boolean} asJson optional flag to force output in json, false by default
     */
    deleteLocal : function(instance, login, asJson) {
        getLocalUser(instance, login, function(err, user) {
            if (err) {
                if (asJson) {
                    console.json({error: err.message});
            ...
            deleteLocalUser(instance, login, user['_resource_state'], function(err) {
                if (err) {
                    if (asJson) {
                        console.json({error: err.message});
                    } else {
