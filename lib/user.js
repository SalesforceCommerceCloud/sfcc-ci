 ...
 */
function searchUsers(instance, query, count, callback) {
    // the page size
    var size = USER_LIST_PAGE_SIZE
    if ( count ) {
        size = Number.parseInt(count);
...
module.exports.cli = {
     ...
     */
    search : function(instance, login, query, count, asJson, sortBy) {
        // get details of a single user if login was passed
        if ( typeof(login) !== 'undefined' && login !== null ) {
        ...
        // get all users
        searchUsers(instance, query, count, function(err, userResult, list) {
            if (err) {
                console.error(err.message);
                return;
            }
