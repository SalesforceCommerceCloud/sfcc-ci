 ...
 */
function searchLocalUsers(instance, query, sortBy, count, callback) {
    // the page size
    var size = USER_LIST_PAGE_SIZE
    if ( count ) {
        size = Number.parseInt(count);
...
module.exports.cli = {
     ...
     * @param {Boolean} asJson optional flag to force output in json, false by default
     */
    searchLocal : function(instance, login, query, sortBy, count, asJson) {
        // get details of a single user if login was passed
        if ( typeof(login) !== 'undefined' && login !== null ) {
        ...
        // get all users
        searchLocalUsers(instance, query, sortBy, count, function(err, userResult, list) {
            if (err) {
                console.error(err.message);
                return;
            }
