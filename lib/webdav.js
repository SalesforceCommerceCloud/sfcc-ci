...
function postFile(instance, path, file, token, ignoreLocalFilePath, options, callback) {
    // temp workarround for larger webdav uploads not working after sandbox has been provisioned
    // touch the BM login url to have the webtier learning about the webdav file upload limit of 100MB
    // can be removed once W-4959586 is fixed
    request.get('https://' + instance + BM_PATH, function (error, response, body) {
