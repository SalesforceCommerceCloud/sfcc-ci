...
function download(instance, file, location, options) {
    downloadFile(instance, file, auth.getToken(), location, options, function (err, res, body) {
        ocapi.ensureValidToken(err, res, function(err, res) {
            // note, server respond with a 401 (Authorization required) in case the WebDAV Client permission is not set
            if (res && res.statusCode >= 400) {
                console.error('Download file %s to %s failed: %s (%s)', file, path, res.statusCode,
                    res.statusMessage);
                return;
            } else if (err) {
                console.error('Download file %s failed: %s', file, err);
                return;
            }
            console.info('File %s downloaded to %s', file, instance);
        }, function() {
...
module.exports.api = {
    ...
    upload : function (instance, path, file, token, options, callback) {
        ...
        /**
     * Downloads an arbitrary file from a Commerce Cloud instance.
     *
     * @param {String} instance The instance to upload the file to
     * @param {String} path The path relative to .../webdav/Sites where the file to upload to
     * @param {String} file The file to upload
