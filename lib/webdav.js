    ...
    postFile(instance, path, file, auth.getToken(), function (err, res, body) {
        ...
        ocapi.ensureValidToken(err, res, function(err, res) {
            ...
            if (res.statusCode >= 400) {
                console.error('Error: Upload file %s to %s failed: %s (%s)', file, path, res.statusCode,
                    res.statusMessage);
                return;
            } else if (err) {
...
function deployCode(instance, archive, sync, callback) {
    var file = archive;
    ...
    postFile(instance, WEBDAV_CODE, file, auth.getToken(), function (err, res, body) {
        ...
        ocapi.ensureValidToken(err, res, function(err, res) {
            // note, server respond with a 401 (Authorization required) in case the WebDAV Client permission is not set
            if (res.statusCode >= 400) {
                console.error('Error: Deploy code %s failed (upload step): %s (%s)', file, res.statusCode,
                    res.statusMessage);
                if (typeof callback !== 'undefined') {
                ...
                console.error('Error: Deploy code %s failed (upload step): %s', file, err);
                if (typeof callback !== 'undefined') {
            ...
            unzip(instance, WEBDAV_CODE, file, auth.getToken(), function (err, res, body) {
                // note, server respond with a 401 (Authorization required) in case the WebDAV Client permission is not set
                if (res.statusCode >= 400) {
                    console.error('Error: Deploy code %s failed (unzip step): %s (%s)', file, res.statusCode,
                        res.statusMessage);
                    if (typeof callback !== 'undefined') {
                    ...
                    console.error('Error: Deploy code %s failed (unzip step): %s', file, err);
                    if (typeof callback !== 'undefined') {
            ...
            deployCode(instance, archive, sync, callback);
        });
    });
}
