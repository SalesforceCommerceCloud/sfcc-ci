...
function upload(instance, path, file, sync, ignoreLocalFilePath) {
    ...
    postFile(instance, path, file, auth.getToken(), ignoreLocalFilePath, function (err, res, body) {
        ...
        ocapi.ensureValidToken(err, res, function(err, res) {
            ...
            upload(instance, path, file, sync, ignoreLocalFilePath);
        });
    });
}
...
function uploadInstanceImport(instance, archive, sync) {
    ...
    // run the upload
    upload(instance, WEBDAV_INSTANCE_IMPEX, file, sync, true);
}
...
function deployCodeCLI(instance, archive, sync) {
    ...
    postFile(instance, WEBDAV_CODE, file, auth.getToken(), false, function (err, res, body) {
        ...
        ocapi.ensureValidToken(err, res, function(err, res) {
            ...
            deployCodeCLI(instance, archive, sync);
        });
    });
}
...
function deployCodeAPI(instance, archive, token, callback) {
    ...
    // initiate the post request first...
    postFile(instance, WEBDAV_CODE, file, token, false, function (err, res, body) {
        ocapi.ensureValidToken(err, res, function(err, res) {
            // note, server respond with a 401 (Authorization required) in case the WebDAV Client permission is not set
...
module.exports.api = {
    ...
    upload : function (instance, path, file, token, callback) {
        ...
        // initiate the request
        postFile(instance, path, file, token, false, function (err, res, body) {
            if (res.statusCode >= 400) {
