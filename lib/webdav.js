...
function deployCodeCLI(instance, archive) {
    ...
    // Set an empty options parameter
    var options = {};
    ...
    postFile(instance, WEBDAV_CODE, file, auth.getToken(), ignoreLocalFilePath, options, function (err, res, body) {
        ocapi.ensureValidToken(err, res, function(err, res) {
            ...
            unzip(instance, WEBDAV_CODE, file, auth.getToken(), ignoreLocalFilePath, options, function (err, res, body) {
                // note, server respond with a 401 (Authorization required) in case the WebDAV Client permission is not set
                if (res && res.statusCode >= 400) {
                ...
                deleteFile(instance, WEBDAV_CODE, file, auth.getToken(), ignoreLocalFilePath, options, function(err, res, body) {
                    if (err) {
