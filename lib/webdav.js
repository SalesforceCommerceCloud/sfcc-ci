...
function postFile(instance, path, file, token, ignoreLocalFilePath, options, callback) {
    ...
    try {
        ...
        // repeat the request (try it again)
        req = request(options, callback);
        fs.createReadStream(file).pipe(req);
    ...
    upload : function (instance, path, file, token, options, callback) {
        ...
        postFile(instance, path, file, token, true, options, function (err, res, body) {
            ...
            if (err) {
                ...
                // If so, callback and throw the error
                callback(new Error(err));

            // Otherwise, was a response provided?
            } else if (res === undefined) {

                // If no response was found, callback with an error
                callback(new Error('No response was found; please re-try the request'));

            // Did the response have a status-code?
            } else if (!res.hasOwnProperty('statusCode')) {

                // If no statusCode was found, then throw a custom error
                callback(new Error('response.statusCode was not found; please re-try the request'));
